...
void print_body_information(k4abt_body_t body) {
  ...
  for (int i = 0; i < (int)K4ABT_JOINT_COUNT; i++) {
    ...
    printf(
        ...
        "%f); Confidence Level (%d) \n",
        i, position.v[0], position.v[1], position.v[2], orientation.v[0],
        orientation.v[1], orientation.v[2], orientation.v[3], confidence_level);
  }
}
...
void print_body_index_map_middle_line(k4a_image_t body_index_map) {
  ...
  body_index_map_buffer =
      body_index_map_buffer +
      middle_line_num * k4a_image_get_width_pixels(body_index_map);
  ...
  while (true) {
    ...
    if (get_capture_result == K4A_WAIT_RESULT_SUCCEEDED) {
      ...
      k4a_wait_result_t queue_capture_result = k4abt_tracker_enqueue_capture(
          tracker, sensor_capture, K4A_WAIT_INFINITE);
